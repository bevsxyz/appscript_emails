 ...
 */
function sendReplyToLastEmailForEachUUID(
  subjectLine,
  ...
  obj.forEach((row) => {
    if (row[UUID_COL] == "") {
      ...
      if (thread) {
        // Ensure we only reply to emails in the inbox
        if (row[REPLY_TIME_COL] == "") {
          const threadId = thread.getId();
          const msgObj = fillInTemplateFromObject_(emailTemplate.message, row);
          const hiddenUuid =
            '<div style="color:white;font-size:1px;">UUID: ' +
            row[UUID_COL] +
            "</div>";
          ...
          draft.update(row[RECIPIENT_COL], subjectLine, msgObj.text, {
            ...
            inlinImages: msgObj.inlineImages,
          });
